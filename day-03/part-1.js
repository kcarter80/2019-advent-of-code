...
for (var wireNumber = 0; wireNumber < wires.length; wireNumber++) {
	...
	for (var i = 0; i < wires[wireNumber].length; i++) {
		...
		switch(wires[wireNumber][i][0]) {
			case 'R':
				...
				while (pointerX < finalX) {
					pointerX += 1;
					setPoint(pointerX,pointerY,wireNumber,matrix);
				}
				break;
			case 'L':
				// code block
		...
		if (finalX < 0) console.log(finalX);
		if (finalY < 0) console.log(finalY);
		//console.log(matrix);
	}
}
findShortestIntersection(matrix, offset);

function setPoint(x,y,wireNumber,matrix) {
	if (matrix[x] == null) {
		matrix[x] = new Array;
	}
	if (matrix[x][y] == null) {
		matrix[x][y] = wireNumber;
	} else if (matrix[x][y] != wireNumber)  {
		matrix[x][y] = 2;
	}
}
	...
	for(let i=0; i<matrix.length; i++) {
		if (matrix[i]) {
			for(let j=0; j<matrix[i].length; j++) {
				if (!(i==offset && j==offset) && matrix[i][j] == 2 && (distance == null || Math.abs(i-offset) + Math.abs(j-offset) < distance)) distance = Math.abs(i-offset) + Math.abs(j-offset);
			}
		}
	}
