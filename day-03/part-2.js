	...
	for (var i = 0; i < wires[wireNumber].length; i++) {
		...
		switch(wires[wireNumber][i][0]) {
			...
			case 'U':
				...
				var finalY = pointerY + distance;
				while (pointerY < finalY) {
					pointerY += 1;
					setPoint(pointerX,pointerY,wireNumber,steps,matrix);
					steps += 1;
				}
...
findShortestIntersection(matrix, offset);

function setPoint(x,y,wireNumber,steps,matrix) {
	if (matrix[x] == null) {
		matrix[x] = new Array;
	}
	if (matrix[x][y] == null) {
		matrix[x][y] = {};
		matrix[x][y][wireNumber] = steps;
	} else if (!matrix[x][y].hasOwnProperty(wireNumber)) {
		matrix[x][y][wireNumber] = steps;
	}
}

function findShortestIntersection(matrix,offset) {
	var distance;
	for(let i=0; i<matrix.length; i++) {
		if (matrix[i]) {
			for(let j=0; j<matrix[i].length; j++) {
				if (!(i==offset && j==offset) && matrix[i][j] != undefined)
					if (matrix[i][j].hasOwnProperty(0) && matrix[i][j].hasOwnProperty(1))
						if  (distance == null || matrix[i][j][0] + matrix[i][j][1] < distance)
							distance = matrix[i][j][0] + matrix[i][j][1];
			}
		}
	}
